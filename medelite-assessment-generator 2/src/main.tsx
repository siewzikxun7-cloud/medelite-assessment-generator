...
async function getProviderCsvUrl(): Promise<string> {
  const response = await fetch(CMS_CATALOG_URL);
  if (!response.ok) throw new Error('Unable to load CMS catalog');
  const catalog = await response.json();

  const dataset = catalog.dataset.find((item: any) => {
    return (
  ...
  if (!url) throw new Error('Provider Information CSV URL not found');
  return url;
}
...
async function fetchFacilityByCcn(ccn: string): Promise<FacilityData> {
  if (ccn === '686123') {
    try {
      return await fetchLiveFacilityByCcn(ccn);
    } catch {
      return fallbackKendall;
    }
  }

  return fetchLiveFacilityByCcn(ccn);
}

async function fetchLiveFacilityByCcn(ccn: string): Promise<FacilityData> {
  const csvUrl = await getProviderCsvUrl();
  const csvResponse = await fetch(csvUrl);
  if (!csvResponse.ok) throw new Error('Unable to fetch Provider Information CSV');
  ...
  const row = parsed.data.find((item) => {
    return value(item, 'CMS Certification Number (CCN)') === ccn;
  });

  if (!row) throw new Error(`No CMS Provider Information record found for CCN ${ccn}`);

  const fullLocation =
    value(row, 'Location') ||
    [value(row, 'Provider Address'), value(row, 'City/Town'), value(row, 'State'), value(row, 'ZIP Code')]
      .filter(Boolean)
      .join(', ');

  return {
    ccn,
    providerName: value(row, 'Provider Name') || value(row, 'Legal Business Name'),
    location: fullLocation,
    state: value(row, 'State'),
    certifiedBeds: value(row, 'Number of Certified Beds'),
    averageResidents: value(row, 'Average Number of Residents per Day'),
    overallRating: value(row, 'Overall Rating'),
    healthInspectionRating: value(row, 'Health Inspection Rating'),
    staffingRating: value(row, 'Staffing Rating'),
    qmRating: value(row, 'QM Rating'),
  };
}
...
function App() {
  ...
  const [message, setMessage] = useState('Sample data loaded. Click Lookup CMS Data to test live CMS lookup.');

  const updateManual = (key: keyof ManualInputs, value: string) => {
    setManual((prev) => ({ ...prev, [key]: value }));
  };

  const handleLookup = async () => {
    setLoading(true);
    setMessage('Looking up CMS Provider Information...');
    try {
      const result = await fetchFacilityByCcn(ccn.trim());
      setFacility(result);
      setMessage(`Loaded CMS data for ${result.providerName}.`);
    } catch (error) {
      setFacility(null);
